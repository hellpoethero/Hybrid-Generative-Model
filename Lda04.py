...
def init(df, df_test, Ks, iters, top_k, datasetName, outFile, folder_name):
	num_record = len(df)
	print(num_record)
	...
	user_index = np.array(df['user_id'].unique())
	test_history = [[0]] * len(nodelist)
	...
	temp_df['user_id'] = user_index
	temp_df['history'] = test_history

	df_test = pd.merge(df_test, temp_df, how='left', left_on=['user_id'], right_on=['user_id']) \
		[['user_id', 'loc_id', 'history', 'repeated', 'count']]

	# run(df, df_test, 2, 1, top_k, datasetName, outFile, folder_name)
	for K in Ks:
		for iteration in iters:
			run(df, df_test, K, iteration, top_k, datasetName, outFile, folder_name)


def run(df, df_test, K, iteration, top_k, datasetName, outFile, folder_name):
	num_record = len(df)
	print(num_record)

	# df_table_testing = df[df['train'] == 0]
	df_table_testing = df_test

	df_inference = df[df['train'] == 1][['user_id', 'loc_id', 'train', 'history']]
	temp = np.random.dirichlet(np.ones(K) * 1000, len(df_inference)).tolist()
	...
	df_table = pd.merge(df_inference, dfgamma, how='left', left_on=['user_id'], right_on=['user_id']) \
		[['user_id', 'loc_id', 'phi', 'train', 'gamma', 'history']]
	...
	# df_inference_testing = df_table_testing[['user_id', 'loc_id', 'history', 'repeated']]
	df_table_testing = pd.merge(df_table_testing, dfgamma, how='left', left_on=['user_id'], right_on=['user_id']) \
		[['user_id', 'loc_id', 'gamma', 'history', 'repeated']]
